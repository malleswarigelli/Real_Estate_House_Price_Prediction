...
# pipeline is built using component, entity files
...
class Pipeline(Thread):
    ...
    def start_data_ingestion(self) -> DataIngestionArtifact:
        '''
        Function takes DataIngestion class from component (parameter is from configuration)
        '''
        try:
            data_ingestion = DataIngestion(data_ingestion_config=self.config.get_data_ingestion_config()) # from configuration
    ...
    # compile pipeline for all components together
    def run_pipeline(self):
        try:
            if Pipeline.experiment.running_status:
                logging.info("Pipeline is already running")
                return Pipeline.experiment # Experiment namedtuple
            ...
            # write the above details in experiment.csv file
            ...
            # stop_time is when pipeline is completed...
            stop_time = datetime.now()
            
            # since pipeline is completed; update values of Experiment namedtuple
            Pipeline.experiment = Experiment(experiment_id=Pipeline.experiment.experiment_id,
                                             initialization_timestamp=self.config.time_stamp,
                                             artifact_time_stamp=self.config.time_stamp,
                                             running_status=False, # since pipeline is completed..
                                             start_time=Pipeline.experiment.start_time,
                                             stop_time=stop_time,
                                             execution_time=stop_time - Pipeline.experiment.start_time,
                                             message="Pipeline has been completed.",
                                             experiment_file_path=Pipeline.experiment_file_path, # path of experiment.csv
    ...
    def save_experiment(self):
        try:
            # if pipeline started, experiment_id is created (its not none)
            if Pipeline.experiment.experiment_id is not None:
                
                experiment = Pipeline.experiment # Experiment namedtuple 
                experiment_dict = experiment._asdict() # convert Experiment namedtuple to dictionary
