...
class ModelPusher:
    ...
    def export_model(self) -> ModelPusherArtifact:
        '''copy trained model into Saved_models folder'''
        try:
            # the trained model that is evaluated file path
            evaluated_model_file_path = self.model_evaluation_artifact.evaluated_model_path
            
            
            # path for saved_models, timestamp 
            
            export_dir = self.model_pusher_config.export_dir_path
            
            # get model file name i.e model.pkl
